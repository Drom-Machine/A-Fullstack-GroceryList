...
export class Grocery extends Component {
  ...
  async componentDidMount() {
    try {
      ...
      let allGroceries = await axios.get(
        "http://localhost:3000/api/groceries/get-all-groceries"
      );
    ...
    if(this.state.groceryInput === ""){
      ...
      try{
        let createdGrocery = await axios.post(
          "http://localhost:3000/api/groceries/create-grocery", {
  ...
  handleDeleteByID = async (id) => {
    try{
      let deletedGrocery = await axios.delete(
        `http://localhost:3000/api/groceries/delete-grocery-by-id/${id}`);
  ...
  handlePurchasedByID = async (id, purchased) => {
    try{
      ...
      `http://localhost:3000/api/groceries/update-grocery-by-id/${id}`, {
        purchased: !purchased,
      });
  ...
  sortByDate = async (order) => {
    try {
      let allGroceries = await axios.get(
        `http://localhost:3000/api/groceries/get-all-groceries/sort-by-date/${order}`
      );
      this.setState({
        groceryList: allGroceries.data.payload,
      });
  ...
  sortByPurchased = async (bool) => {
    try {
      let allGroceries = await axios.get(
        `http://localhost:3000/api/groceries/get-all-purchased/${bool}`
      );
      this.setState({
        groceryList: allGroceries.data.payload,
      });
